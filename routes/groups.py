...
from services.messaging import handle_send_sticker, handle_send_voice
...
def group_messages(chat_id_raw):
    ...
    with db_session() as session:
        ...
        # Optional "Mentions" filter: only messages mentioning the operator.
        if request.args.get("mentions_only") in ("1", "true", "yes"):
            base = base.filter(TgMessage.mentions_operator.is_(True))
...
@bp.post("/<chat_id_raw>/messages")  # alias: same behaviour, panel-friendly path
@require_panel_auth
...
@bp.post("/<chat_id_raw>/sticker")
@bp.post("/<chat_id_raw>/stickers")  # alias (plural)
@require_panel_auth
def send_group_sticker(chat_id_raw):
    """Send a sticker (by Telegram file_id) into a group/supergroup."""
    chat_id = parse_chat_id(chat_id_raw)
    if chat_id is None:
        return error(400, "Invalid chat id")
    return handle_send_sticker(chat_id, "group")


@bp.post("/<chat_id_raw>/voice")
@require_panel_auth
def send_group_voice(chat_id_raw):
    """Send a voice note (multipart 'voice' upload) into a group."""
    chat_id = parse_chat_id(chat_id_raw)
    if chat_id is None:
        return error(400, "Invalid chat id")
    return handle_send_voice(chat_id, "group")
