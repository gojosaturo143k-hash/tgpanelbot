...
def _stream_telegram_file(file_id):
    """Resolve + stream a Telegram file. Shared by both media routes."""
    try:
...
@bp.get("/telegram/<path:file_id>")
@require_panel_auth
def media_file_authenticated(file_id):
    """
    Bearer-authenticated media proxy (same PANEL_API_KEY as every other
    /api endpoint). Use this from panel code that can set headers, e.g.
    fetch() + object URLs.
    """
    return _stream_telegram_file(file_id)


@bp.get("/<path:file_id>")
def media_file(file_id):
    """
    Signature-authenticated media proxy for direct <img>/<audio> tags,
    which cannot send an Authorization header. The signed, expiring URL
    is generated server-side and embedded in message payloads.
    """
    if not verify_media_signature(
        file_id, request.args.get("exp"), request.args.get("sig")
    ):
