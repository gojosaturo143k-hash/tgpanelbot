...
@require_panel_auth
def send_dm_sticker(chat_id_raw):
    """Send a sticker (by Telegram file_id) into a private conversation."""
    chat_id = parse_chat_id(chat_id_raw)
    if chat_id is None:
        return error(400, "Invalid chat id")
    return handle_send_sticker(chat_id, "private")


@bp.post("/<chat_id_raw>/voice")
@require_panel_auth
def send_dm_voice(chat_id_raw):
    """Send a voice note (multipart 'voice' upload) into a private chat."""
    chat_id = parse_chat_id(chat_id_raw)
    if chat_id is None:
        return error(400, "Invalid chat id")
    return handle_send_voice(chat_id, "private")
