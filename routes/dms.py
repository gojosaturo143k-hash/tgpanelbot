...
def reply_to_dm(chat_id_raw):
    ...
    log.info("Web Panel sending private DM reply (chat_id=%s)", chat_id)
    try:
        telegram_result = send_message(chat_id, text)
    ...
    stored = None
    try:
        stored = save_outgoing_message(chat_id, telegram_result, text)
