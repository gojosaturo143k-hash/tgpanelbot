...
def dashboard():
    with db_session() as session:
        ...
        # Same enriched serialization as the message endpoints, so sender
        # names, reply relationships and media render correctly here too.
        recent_activity = []
        for entry in serialize_messages(session, recent_rows):
            chat = chats.get(entry["telegram_chat_id"])
