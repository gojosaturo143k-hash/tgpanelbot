...
class TgMessage(Base):
    ...
    # Telegram message entities (mention, text_mention, bold, url, ...)
    # stored verbatim so original offsets/lengths are never destroyed.
    entities = Column(JSONB, nullable=True)

    reply_to_telegram_message_id = Column(BigInteger, nullable=True)

    # Snapshot of the replied-to message taken at receive time. Lets the
    # panel render a reply preview even when the original message is not
    # in our database (sent before the bot joined, or deleted since).
    reply_metadata = Column(JSONB, nullable=True)

    # True when the configured panel operator was mentioned. Always False
    # when no operator identity is configured.
    mentions_operator = Column(
    ...
    def to_dict(self):
        ...
        return {
            ...
            "media": self.media,
            # Raw Telegram entities with original offsets/lengths.
            "entities": self.entities or [],
            "mentions_operator": bool(self.mentions_operator),
            # Alias kept so either naming works for API consumers.
            "mentioned_operator": bool(self.mentions_operator),
            "telegram_timestamp": (
...
def _entity_text(text, entity):
    """Slice the exact entity substring using Telegram's UTF-16 offsets."""
    if text is None:
        return None
    ...
    except Exception:  # noqa: BLE001 - never break serialization on odd input
        return None


def mentions_payload(message, users_by_username, users_by_id):
    """
    Normalized mention list for the panel.

    Resolves mentions to known Telegram users when possible; otherwise the
    raw @username/text is preserved and NO user is invented.
    """
    ...
    operator_id = Config.operator_user_id()
    operator_username = (Config.OPERATOR_TELEGRAM_USERNAME or "").lower()
    ...
    for entity in entities:
        ...
        raw = _entity_text(message.text, entity)
        user = None
        username = None
        embedded = entity.get("user") or {}

        if etype == "mention":
            username = (raw or "").lstrip("@") or None
            if username:
                user = users_by_username.get(username.lower())
        else:
            # text_mention carries the Telegram user object inline, which
            # is the only way to resolve users that have no @username.
            user = users_by_id.get(embedded.get("id"))
            username = embedded.get("username") or (user.username if user else None)

        user_id = user.telegram_user_id if user else embedded.get("id")

        if user is not None:
            display_name = sender_display_name(user, user_id)
        elif embedded:
            # Use the name Telegram itself supplied rather than an id.
            parts = [p for p in [embedded.get("first_name"), embedded.get("last_name")] if p]
            display_name = " ".join(parts) or embedded.get("username") or raw
        else:
            # Unknown @username: keep the raw text, invent nothing.
            display_name = raw

        is_operator = False
        if operator_id and user_id and int(user_id) == operator_id:
            is_operator = True
        elif operator_username and username and username.lower() == operator_username:
            is_operator = True
...
def serialize_messages(session, messages):
    ...
    - "reply_to_message_id" + "reply_to": the replied-to message with its
      own sender, text and media type (falls back to the stored
      reply_metadata snapshot when the original is not in our database)
    - "entities" + "mentions": mention ranges resolved to known users
    """
    rows = list(messages)

    user_ids = {m.telegram_user_id for m in rows if m.telegram_user_id is not None}

    # Usernames referenced by @mention entities, and user ids embedded in
    # text_mention entities - resolved in one batch each.
    ...
    for message in rows:
        for entity in (message.entities or []):
            ...
            if etype == "mention":
                raw = _entity_text(message.text, entity)
                if raw:
                    mention_usernames.add(raw.lstrip("@").lower())
            elif etype == "text_mention":
                uid = (entity.get("user") or {}).get("id")
                if uid is not None:
                    user_ids.add(uid)
    reply_keys = {
    ...
    if mention_usernames:
        for user in (
            ...
            .filter(func.lower(TgUser.username).in_(mention_usernames))
            .all()
        ):
            if user.username:
                users_by_username[user.username.lower()] = user
                users.setdefault(user.telegram_user_id, user)
    ...
    for message in rows:
        ...
        data["mentions"] = mentions_payload(message, users_by_username, users)
        ...
        # Flat convenience aliases for media messages, so the frontend can
        # read a file id without digging into the nested object.
        media = message.media or {}
        data["file_id"] = media.get("file_id")
        data["file_unique_id"] = media.get("file_unique_id")
        ...
        if reply_id is not None:
            ...
            elif message.reply_metadata:
                # Original is not in our database (older than the bot, or
                # deleted) - use the snapshot captured at receive time so
                # the panel can still render a proper preview.
                snapshot = dict(message.reply_metadata)
                snapshot.setdefault("telegram_message_id", reply_id)
                snapshot.setdefault("telegram_chat_id", message.telegram_chat_id)
