...
def process_update(update):
    ...
    Phase 1 supports text messages from private chats, groups and
    supergroups. No automatic reply is ever sent.
    """
    ...
    if text is None:
        log.info(
            "Skipping non-text Telegram message (chat_id=%s, message_id=%s); "
    ...
    if chat_type in GROUP_TYPES:
        log.info(
            "Telegram group message received (chat_id=%s, message_id=%s)",
            chat_id,
            message_id,
        )
    else:
        log.info(
            "Telegram private DM received (chat_id=%s, message_id=%s)",
            chat_id,
            message_id,
        )
...
def save_outgoing_message(chat_id, telegram_result, text):
    """
