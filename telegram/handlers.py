...
def _extract_entities(message):
    """
    Preserve Telegram message entities verbatim (offsets/lengths intact).

    Uses `entities` for text messages and `caption_entities` for captions,
    so mention ranges survive exactly as Telegram sent them.
    """
    ...
    for entity in entities:
        ...
        if user:
            # text_mention: the only way to resolve users without @username
            item["user"] = {
                "id": user.get("id"),
...
def _mentions_operator(message, entities):
    """
    True when the configured panel operator is mentioned.

    Returns False when no operator identity is configured - an identity is
    never invented.
    """
    operator_id = Config.operator_user_id()
    operator_username = (Config.OPERATOR_TELEGRAM_USERNAME or "").lower()
    if not operator_id and not operator_username:
        return False
    ...
    for entity in entities or []:
        ...
        if etype == "text_mention":
            ...
            if operator_id and uid and int(uid) == operator_id:
                return True
        elif etype == "mention" and operator_username:
            offset, length = entity.get("offset"), entity.get("length")
            if offset is None or length is None:
                continue
            try:
                encoded = text.encode("utf-16-le")
                raw = encoded[offset * 2: (offset + length) * 2].decode("utf-16-le")
            except Exception:  # noqa: BLE001
                continue
            if raw.lstrip("@").lower() == operator_username:
                return True
    return False
...
def _reply_snapshot(message):
    """
    Capture a lightweight preview of the replied-to message.

    Stored at receive time so the panel can render "Replying to ..." even
    if the original message is not (or no longer) in our database.
    """
    ...
    parts = [p for p in [sender.get("first_name"), sender.get("last_name")] if p]
    display_name = (
        " ".join(parts) or sender.get("username") or
        (str(sender["id"]) if sender.get("id") else None)
    )

    if original.get("text") is not None:
        original_type, text = "text", original.get("text")
    elif original.get("sticker"):
        original_type, text = "sticker", None
    elif original.get("voice"):
        original_type, text = "voice", original.get("caption")
    else:
        original_type, text = "other", original.get("caption")

    return {
        "telegram_message_id": original.get("message_id"),
        "telegram_chat_id": (original.get("chat") or {}).get("id"),
...
def process_update(update):
    ...
    mentions_operator = _mentions_operator(message, entities)
    if mentions_operator:
        log.info(
            "Panel operator was mentioned (chat_id=%s, message_id=%s)",
            chat_id,
            message_id,
        )
...
def _outgoing_media(telegram_result, message_type):
    """Build the stored media block from Telegram's sendX result."""
    if message_type == "sticker":
        sticker = telegram_result.get("sticker") or {}
        return {
            "file_id": sticker.get("file_id"),
            "file_unique_id": sticker.get("file_unique_id"),
            "type": sticker.get("type"),
    ...
    if message_type == "voice":
        voice = telegram_result.get("voice") or {}
        return {
            "file_id": voice.get("file_id"),
            "file_unique_id": voice.get("file_unique_id"),
            "duration": voice.get("duration"),
...
def save_outgoing_message(
    ...
    with db_session() as session:
        ...
        record = TgMessage(
            ...
            sender_type="admin",
            text=text,
            # Real type of what we sent - never blindly "text".
            message_type=message_type,
            media=_outgoing_media(telegram_result, message_type),
            reply_to_telegram_message_id=reply_to_message_id,
            reply_metadata=_reply_snapshot(telegram_result),
        ...
        try:
            ...
            raise DuplicateMessageError()
        # Return the same enriched shape the message list endpoints use
        # (sender, sticker/voice with URLs, reply_to, mentions).
        return serialize_messages(session, [record])[0]
